...
type FormInputTextComponent = React.ForwardRefExoticComponent<
  FormInputTextProps & React.RefAttributes<HTMLInputElement>
>;

// Define the component
const FormInputText: FormInputTextComponent = forwardRef<HTMLInputElement, FormInputTextProps>(
  function ForwardFormEntry({ type, placeholder, label, error, ...rest }, ref) {
    const [switchPasswordInput, setSwitchPasswordInput] = useState(false);
    const isPassword = type === 'password';

    return (
      <div className="relative">
        {label && <label className="block text-sm font-medium uppercase text-gray-500">{label}</label>}
        <input
        ...
        {error && (
          <div className="mt-1 flex items-center gap-2 text-red-500">
            <svg className=" h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <circle cx="12" cy="12" r="10" stroke-width="1.5" className="stroke-red-500" />
              <path d="M12 17V11" stroke-width="1.5" strokeLinecap="round" className="stroke-red-500" />
              <circle cx="1" cy="1" r="1" transform="matrix(1 0 0 -1 11 9)" className="stroke-red-500" />
            </svg>
