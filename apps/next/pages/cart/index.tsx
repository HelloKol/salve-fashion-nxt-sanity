...
import { Button, Container, FormInputText, Grid, ImageTag, Main, Section, Seo } from '@/components';
...
export default function Page({ page }: props): JSX.Element | null {
  ...
  const { cartLoading, cart, cartId, lineItemRemove, lineItemUpdateQuantity } = useShoppingCart();
  const {
  ...
  const handleClipboard = async (handle: string) => {
    await copyToClipboard(env.NEXT_PUBLIC_BASE_URL + `/shop/product/${handle}`);
  };
  ...
  const renderDiscountCode = () =>
    cart?.cart?.discountCodes.map((discount: { applicable: boolean; code: string }, index) => {
      const { applicable, code } = discount;
      if (!applicable) return null;

      return (
        <div key={index} className="flex items-center rounded-full border-[1px] border-black p-1">
          {code}
        ...
        <div key={id} className="mt-8 flex gap-6 lg:mb-10 lg:mt-0">
          ...
          <div className={`flex flex-col justify-between`}>
            <div>
              ...
              <div className="mt-2">{renderVariantOptions(selectedOptions)}</div>
            </div>

            <div className="flex items-center justify-between">
              <div className="flex items-center gap-5 rounded-full border-[1px] border-black px-3 py-1">
                <button onClick={() => lineItemUpdateQuantity(id, quantity - 1)} disabled={quantity <= 1}>
                  <svg
                    className="h-4 w-4 text-gray-800"
                    aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <span>{quantity}</span>
                <button onClick={() => lineItemUpdateQuantity(id, quantity + 1)} disabled={quantity >= 100}>
                  <svg
                    className="h-4 w-4 text-gray-800"
                    aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg"
          ...
          <div className={`ml-auto flex flex-col justify-between`}>
            <p className="lg:text-md text-sm font-semibold">£{subtotalAmount.amount}</p>
    ...
    <>
      ...
      <Main>
        <Section withPadding={false}>
          <Container>
            <Grid>
              <button className="col-span-full flex w-fit items-center gap-4" onClick={goBack}>
                <svg
                ...
                <h1 className="text-3xl md:text-4xl xl:text-5xl">Shopping Bag</h1>
              </button>
              ...
              <div className="col-span-full my-14 md:col-start-3 md:col-end-11 lg:col-start-10 lg:col-end-13 lg:my-0 lg:mt-14">
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                  ...
                  <div className="flex items-center">{renderDiscountCode()}</div>
                ...
                <p className="mb-2 mt-6 flex justify-between uppercase">
                  <span>bag total</span> ${cart?.cart?.cost?.subtotalAmount?.amount}{' '}
                </p>
                <p className="mb-2 flex justify-between uppercase">
                  <span>Tax</span> ${cart?.cart?.cost?.totalTaxAmount?.amount || `0.00`}{' '}
                </p>
                <p className="mb-2 flex justify-between uppercase">
                  <span>Shipping</span>${cart?.cart?.cost?.totalDutyAmount?.amount || `0.00`}{' '}
                </p>
                <p className="mb-2 flex justify-between uppercase">
                  <span>Delivery</span> ${cart?.cart?.cost?.totalDutyAmount?.amount || `0.00`}{' '}
                </p>
                <p className="mt-4 flex justify-between border-t-[1px] border-black pt-4 text-lg font-semibold uppercase">
                  <span>total</span> ${cart?.cart?.cost?.checkoutChargeAmount?.amount}{' '}
                </p>
